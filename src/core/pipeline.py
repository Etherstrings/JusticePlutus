...
class StockAnalysisPipeline:
    ...
    def __init__(
        ...
        self.run_output_dir = self._resolve_output_dir()
    ...
    def _resolve_output_dir(self) -> Optional[Path]:
        ...
        raw_output_dir = getattr(self.config, "report_output_dir", None)
        if not raw_output_dir:
            return None
        return Path(raw_output_dir)

    def _write_text_artifact(self, relative_path: str, content: str) -> Optional[Path]:
        """Write a markdown/text artifact for the current run."""
        if not self.run_output_dir:
            return None
        destination = self.run_output_dir / relative_path
        destination.parent.mkdir(parents=True, exist_ok=True)
        destination.write_text(content, encoding="utf-8")
        return destination

    def _write_json_artifact(self, relative_path: str, payload: Dict[str, Any]) -> Optional[Path]:
        """Write a JSON artifact for the current run."""
        if not self.run_output_dir:
            return None
        destination = self.run_output_dir / relative_path
        destination.parent.mkdir(parents=True, exist_ok=True)
    ...
    def _persist_single_stock_artifacts(
        self,
        result: AnalysisResult,
        report_content: str,
    ) -> None:
        """Persist per-stock markdown and JSON outputs immediately after analysis completes."""
        self._write_text_artifact(f"stocks/{result.code}.md", report_content)
        self._write_json_artifact(f"stocks/{result.code}.json", result.to_dict())
        ...
        try:
            ...
            if result:
                ...
                try:
                    ...
                    self._persist_single_stock_artifacts(result, report_content)
                    ...
                    if single_stock_notify:
                        if self.notifier.is_available():
                            ...
                            logger.info(f"[{code}] 未配置通知渠道，已仅保存单股报告")
                except Exception as e:
                    logger.error(f"[{code}] 单股报告处理异常: {e}")
    ...
    def _send_notifications(
        ...
        try:
            ...
            summary_filename = "summary.md" if self.run_output_dir else None
            # 保存到本地
            filepath = self.notifier.save_report_to_file(report, filename=summary_filename)
